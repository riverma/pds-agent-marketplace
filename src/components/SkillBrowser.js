...
const SkillCard = ({
  ...
  const getLocalPath = (skill) => {
    ...
    if (isLocalhost) {
      return `[YOUR_REPO_PATH]/static/marketplace/skills/${skill.name}`;
    }
  ...
  const getRepositoryInfo = () => {
    ...
    if (hostname.endsWith(".github.io")) {
      ...
      const pathParts = window.location.pathname
        ...
        .filter((part) => part);
      const projectName =
        pathParts.length > 0 ? pathParts[0] : siteConfig.projectName;
    ...
    // Fallback for other deployments or localhost — derived from site config
    return {
      organization: siteConfig.organizationName,
      project: siteConfig.projectName,
      githubUrl: `https://github.com/${siteConfig.organizationName}/${siteConfig.projectName}`,
      baseUrl: baseUrl,
      assetsUrl: `${baseUrl}${siteConfig.baseUrl}assets/zip`,
    };
  };
  ...
  const getClaudeCodeCommand = (item) => {
    if (item.type === "skill") {
      ...
      return `/plugin install ${item.name}@${marketplaceName}`;
    } else if (item.type === "mcp") {
      ...
      if (item.npm_package) {
        ...
        return `/plugin install ${pkgName}@${marketplaceName}`;
      }
  ...
  const getMarketplaceCommand = (skill) => {
    ...
    if (isLocalhost) {
      ...
      return "/plugin marketplace add [YOUR_REPO_PATH]";
    }
  ...
  const getGitCloneCommand = (item) => {
    ...
    if (isLocalhost) {
      ...
      return `cp -r [YOUR_REPO_PATH]/static/marketplace/${pathType}/${item.name} ./`;
    } else {
...
const SkillBrowser = ({ searchTerm, setSearchTerm, isSearchActive }) => {
  ...
  const [baseUrl, setBaseUrl] = useState(siteConfig.url);
  const [localMarketplacePath, setLocalMarketplacePath] = useState("");
  const [highlightedSkill, setHighlightedSkill] = useState(null);

  // Marketplace name used in plugin-install commands (from registry.marketplace.name)
  ...
  useEffect(() => {
    ...
    const registries = siteConfig.customFields?.marketplaceConfig?.registries || [
      ...
      .then((data) => {
        ...
        const getEnvironment = () => {
          ...
          return {
            ...
            baseUrl: isLocal ? "http://localhost:3000" : siteConfig.url,
          };
        };
        ...
        // For remote registries, resolve registry-relative skill_file_url
        // paths against the remote origin so documentation links resolve
        // correctly instead of against this site's own origin.
        const isRemoteRegistry =
          registryPath.startsWith("http://") ||
          registryPath.startsWith("https://");
        if (isRemoteRegistry) {
          const remoteOrigin = getRegistryBaseUrl(registryPath);
          combined.forEach((item) => {
            if (
              typeof item.skill_file_url === "string" &&
              item.skill_file_url.startsWith("/")
            ) {
              item.skill_file_url = `${remoteOrigin}${item.skill_file_url}`;
            }
          });
        }

        // Marketplace name (used in plugin-install commands)
        setMarketplaceName(data.marketplace?.name || "marketplace");
          ...
          branding.shouldShowEmptyState() ? (
            <Card className="text-center p-5 shadow-sm">
              <Card.Body>
                ...
                >
                  Get started by adding your own best practices, or by
                  federating other marketplaces in your site configuration.
                </p>
              </Card.Body>
            </Card>
          ) : (
