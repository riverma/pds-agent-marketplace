/**
 * File Browser Component for the Best Practices Marketplace
...
// Docusaurus baseUrl (e.g. "/slim-framework/") — used to build static asset URLs
...
const FileBrowser = ({ itemType = 'skill', itemName, basePath }) => {
  ...
  const buildFileTree = useCallback(async () => {
    ...
    try {
      ...
      const manifestResponse = await fetch(`${BASE_URL}data/marketplace-files.json`);
      ...
      const baseUrl = `${BASE_URL}marketplace/${category}/${itemName}`;
    ...
    return (
      <Alert variant="warning" className="mb-3">
        ...
        <Button
          ...
          size="sm"
          onClick={() => window.open(`${BASE_URL}marketplace/${itemType}s/${itemName}/${itemType.toUpperCase()}.md`, '_blank')}
        >
