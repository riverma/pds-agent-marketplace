 ...
 *
 * Helper functions for managing the marketplace branding configuration.
...
export const getBrandingConfig = (siteConfig) => {
  ...
  return {
    ...
    // External registries are handled by siteConfig.customFields.marketplaceConfig.registries
    getExternalRegistries: () => siteConfig.customFields?.marketplaceConfig?.registries?.filter(url => url.startsWith('http')) || [],
...
/**
 * Update external registries in marketplaceConfig
...
export const updateExternalRegistries = (existingConfig, newRegistries) => {
  const localRegistries = existingConfig.customFields?.marketplaceConfig?.registries?.filter(url => !url.startsWith('http')) || [];
  ...
  return {
    ...
    customFields: {
      ...
      marketplaceConfig: {
        ...existingConfig.customFields?.marketplaceConfig,
    ...
    customFields: {
      ...
      brandingConfig: {
        ...
        marketplace: {
          ...
          // Note: External registries are handled via marketplaceConfig.registries
        }
      }
    }
  };
...
export const validateBrandingConfig = (brandingConfig) => {
  ...
  if (brandingConfig.marketplace) {
    ...
    // Note: External registries are validated as part of marketplaceConfig.registries
  }
